...
export function GridSkeleton({ itens = 12 }: { readonly itens?: number }) {
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
      {Array.from({ length: itens }).map(() => (
        <div key={crypto.randomUUID()} className="glass flex animate-pulse flex-col gap-3 rounded-2xl p-4">
